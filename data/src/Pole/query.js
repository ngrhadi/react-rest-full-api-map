...
const getPole = async () => {
    $results = await ('SELECT row_to_json(f) As feature FROM (SELECT * , ST_AsGeoJSON(geom)::json As geometry, row_to_json((SELECT l FROM (SELECT device_id AS device_id) As l)) As properties FROM public."Pole") As f');
    return $results; 
